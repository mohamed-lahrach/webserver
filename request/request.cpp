...
Request::Request() : http_method(""), requested_path(""), http_version(""),
	got_all_headers(false), expected_body_size(0), request_body(""), config(0),
	location(0), get_handler(), post_handler(), delete_handler()
{
...
{
	LocationContext	*matched_location;
	size_t			longest_len;
	...
	while (it_location != config->locations.end())
	{
		if (it_location->path[it_location->path.length() - 1] == '/'
			&& it_location->path.length() > 1)
			it_location->path = it_location->path.substr(0,
					it_location->path.length() - 1);
		std::cout << "  Checking location: '" << it_location->path << "'" << std::endl;
		if (resquested_path.compare(0, it_location->path.length(),
				it_location->path) == 0)
		{
			if (it_location->path == "/"
...
RequestStatus Request::add_new_data(const char *new_data, size_t data_size)
{
	...
	if (!got_all_headers)
	{
		headers_end_position = incoming_data.find("\r\n\r\n");
		if (headers_end_position == std::string::npos)
		{
			...
			std::cout << "Headers are not complete yet
				- waiting for more data" << std::endl;
			return (NEED_MORE_DATA);
		}
		std::cout << "Found all headers! Now reading them..." << std::endl;
		std::string just_the_headers = incoming_data.substr(0,
				headers_end_position);
		...
		std::cout << "HTTP Method: " << http_method << ",
			...
			Version: " << http_version << std::endl;
		if (http_method == "POST")
		{
			std::map<std::string,
				std::string>::iterator it_content_len = http_headers.find("content-length");
			std::map<std::string,
				std::string>::iterator it_transfer_enc = http_headers.find("transfer-encoding");
				...
				&& it_transfer_enc->second.find("chunked") != std::string::npos)
			{
				...
				std::cout << "Using chunked transfer encoding
...
bool Request::check_for_valid_http_start()
{
	...
	first_line_end = incoming_data.find("\r\n");
...
bool Request::parse_http_headers(const std::string &header_text)
{
	...
	{
		colon_position = current_line.find(':');
...
RequestStatus Request::figure_out_http_method()
{
	...
	// Check if this is a CGI request first,
		before handling with regular handlers
	if (!location->cgiExtensions.empty() && !location->cgiPaths.empty())
	{
		...
		for (size_t i = 0; i < location->cgiExtensions.size(); ++i)
		{
			...
			{
				std::string file_ext = requested_path.substr(requested_path.size()
						- ext.size());
				if (file_ext == ext)
				{
					if (http_method == "POST")
					{
						...
						return post_handler.handle_post_request(http_headers,
							incoming_data, expected_body_size, config, location,
							requested_path);
					}
	...
	{
		return post_handler.handle_post_request(http_headers, incoming_data,
			expected_body_size, config, location, requested_path);
	}
...
{
	if (!location || location->cgiExtensions.empty()
		|| location->cgiPaths.empty())
	{
		return false;
	}
